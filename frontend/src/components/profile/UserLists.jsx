  ...
  return (
    <div className="bg-white rounded-2xl shadow-md p-6">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-bold text-gray-800">Listas</h2>
        {userLists.length > 0 && (
          <Link 
      ...
      {userLists.length > 0 ? (
          ...
          {userLists.slice(0, 4).map((list) => {
            ...
            return canClick ? (
              <Link 
                key={list.id} 
                {...linkProps}
                className="block border border-gray-200 rounded-lg p-4 hover:shadow-md transition hover:border-indigo-300"
              >
              ...
              <div
                key={list.id}
                className="block border border-gray-200 rounded-lg p-4 bg-gray-50 text-gray-400 cursor-not-allowed"
              >
                <ListCardContent list={list} />
              </div>
            );
        ...
        <div className="text-center py-8">
          ...
          <p className="text-gray-500">Este usuário ainda não criou nenhuma lista.</p>
    ...
    <>
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">{list.items?.length || 0} itens</p>
        </div>

...
export default function GenreFilterSection({ 
  ...
  selectedGenres, 
  onGenreChange,
  label = "gênero" // 👈 novo prop com fallback
}) {
  const [searchTerm, setSearchTerm] = useState("");
  const [showAll, setShowAll] = useState(false);

  // Filtra e limita gêneros/plataformas
  ...
  return (
    <div className="space-y-4 scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-gray-700">
      ...
      <div className="relative">
        <input
          type="text"
          placeholder={`Buscar ${label}s...`} // 👈 usa a label
          value={searchTerm}
      ...
      {/* Contador e botão Mostrar Mais */}
      <div className="flex justify-between items-center text-sm text-gray-400">
        <span>
          {selectedGenres.length} de {genres.length} {label}s selecionados
        </span>
      ...
      {selectedGenres.length > 0 && (
        <div className="bg-blue-900/20 border border-blue-700/50 rounded-lg p-3">
          <h4 className="text-sm font-medium text-blue-300 mb-2">
            {label[0].toUpperCase() + label.slice(1)}s selecionados:
          </h4>
